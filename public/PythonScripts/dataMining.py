...
df_questions = df.filter(regex=r"^Q").dropna(axis=1, how="all")
...
df_by_number_of_test_passes.columns = [
    "_".join(x) for x in df_by_number_of_test_passes.columns
]
    ...
    json.dumps(
        {
            ...
            "data_frame_describe": df.describe(
            ...
            "number_of_test_passes_hist_count": json.dumps(
                number_of_test_passes_hist_count.tolist()
            ),
            "number_of_test_passes_hist_division": json.dumps(
                number_of_test_passes_hist_division.tolist()
            ),
            # score by year
            "score_by_year": df_score_by_year.to_json(
                date_format="iso", orient="split"
            ),
            # questions
            "questions_describe": df_questions.describe().to_json(),
            "question_outliers": df_questions.apply(
                lambda x: x.dropna()[filter_not_outliers].unique().tolist(), axis=0
            ).to_json(),
            "question_hists": json.dumps(question_hists),
            # number of questions
            "number_of_questions_describe": (
                df["number_of_questions"].describe().to_json()
            ),
            "number_of_questions_outliers": json.dumps(
                df["number_of_questions"][filter_not_outliers].unique().tolist()
            ),
            # score, spent_time_m by number_of_test_passes
